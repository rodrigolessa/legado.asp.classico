...
DIM nome, nomeArquivo, tamanho, modelo, modelo_galeria, modelo_video
DIM tipo, extArquivo, ultRegistro, aModeloVideo
...
If cmd = "S" Then
	...
	if extArquivo <> "FLV" then
		erro = "Arquivo informado (" & extArquivo & ") não é do tipo esperado, deve ser 'flv'!"
	end if
	
	nomeArquivo = lCase("v_mod" & modelo & "_" & modelo_video & "." & extArquivo)
		...
		If oper = "I" Then
			...
			If countFiles > 0 Then
					...
					If fs.FileExists(mainDir & nomeArquivo) Then
						...
						sSQL = "UPDATE XCA_modelos_videos SET nomeArquivo = '" & nomeArquivo & "', tamanhoArquivo = '" & tamanho & "' WHERE modelo_video = " & modelo_video & " "
						conn.execute(sSQL)
...
<table width="280" border="0" cellpadding="0" cellspacing="10" class="texto2">
	<form action="dlgVideo_incluir.asp?upFile=true" method="post" enctype="multipart/form-data">
	...
	<tr>
		<td colspan=2>Cadastro do vídeo no formato do FLASH (*.flv)</td>
	</tr>
