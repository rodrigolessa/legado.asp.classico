...
Dim query
Dim calendario, dia, mes, ano, titulo, descricao, tipo, status, notExpira
Dim sTitulo, sDescricao, sTexto
Dim data, hora, dataExpiracao, horaExpiracao, sData, sDataExpiracao
Dim oper, cmd, erro, msg_erro
Dim s, i

'variaveis de filro
Dim filtro, fTexto, fCampo, fStatus, fTipo
...
Const nomeTabela = "XCA_calendarios"
...
<head>
	...
	function getOption() {
		...
		f.calendario.value = 0;
		if (f.registroSel.length != undefined) {
			for (var i = 0; i < f.registroSel.length; i++) {
				if (f.registroSel[i].checked) { f.calendario.value = f.registroSel[i].value }
			}
		} else {
		f.calendario.value = f.registroSel.value
		}
	}
	...
	function editarLinha(a) {
		...
		document.forms[0].calendario.value=a;
		document.forms[0].submit();
	}
	...
	function excluir() {
		...
		for (var i = 0; i < f.elements.length; i++) {
			if ((f.elements[i].name=="acalendario") && (f.elements[i].checked)) {
				intDels++;
			}
		}
		if (intDels>0) {
			if (confirm("Você está prestes a apagar " + intDels + " registros.\n Tem certeza que deseja continuar?")) {
				document.forms[0].action="calendarios_excluir.asp";
				document.forms[0].submit();
			}
		} else {
	...
	function altImagem(cmd, campo, chave, titulo) {
		if (document.forms[0].calendario.value>0) {
			//verifico se é na inclusao ou se é na edição do registro.
			var sURL = "dlgAltImagem.asp?cmd=" + cmd + "&nomeTabela=<%=nomeTabela%>&nomeCampoChave=calendario&nomeCampo=" + campo + "&chave=" + chave + "&titulo=" + titulo;
			var sName = "wDlgAlterarImagem";
			var features = "left=300,top=200,width=140,height=250";
		  var _WImg=window.open(sURL, sName, features);
			  _WImg.focus();
		} else {
			if (confirm("É preciso salvar a calendario antes.\nDeseja salvar agora?")) {
	...
	function altFoto(c, a, tipo, n) {
		//verifico se é na inclusao ou se é na edição do registro.
		var sURL = "dlgAltFoto.asp?cmd=" + c + "&nomeTabela=<%=nomeTabela%>&nomeCampo=calendario&chave=" + a + "&titulo=" + n + "&tipo=" + tipo;
...
-->	
		...
		<div id="principal">
			...
			<div id="janela">
				...
				<div id="titulo">Cadastro de dias do calendário</div>
				...
				<div id="ferramenta">
					...
					%>
						...
						<select name="fCampo" size="1" class="sel">
							...
							<option value="dia" <% If fCampo = "dia" Then Response.Write " SELECTED " %>>Dia
							<option value="mes" <% If fCampo = "mes" Then Response.Write " SELECTED " %>>Mês
							<option value="ano" <% If fCampo = "ano" Then Response.Write " SELECTED " %>>Ano
						</select>
						<select name="fTipo" size="1" class="sel">
							<option value="">Tipo
							<option value="N" <% If fTipo = "N" Then Response.Write " SELECTED " %>>Não se repete
							<option value="A" <% If fTipo = "A" Then Response.Write " SELECTED " %>>Repete anualmente
							<option value="M" <% If fTipo = "M" Then Response.Write " SELECTED " %>>Repete mensalmente
						</select>
			...
			<%	end if	%>
				...
				<div id="conteudo">
					<% If oper = "I" OR oper = "E" Then %>
					<!-- #include file="calendarios_incluir_editar.asp"-->
					<% Else %>
					<!-- #include file="calendarios_lista.asp"-->
					<% End If %>
				</div>
