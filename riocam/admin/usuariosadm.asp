...
DIM msg_erro
Dim query
Dim usuario, registroSel, login, senha, email, grupo, descGrupo, adm, status
Dim senha2
Dim oper, cmd, erro
Dim s, i, sGrupo
DIM strRedir
...
Const nomeTabela = "XCA_usuariosADM"
...
if len(trim(request("oper"))) > 0 then oper = request("oper") else oper = "L"

if len(trim(request("cmd"))) > 0 then cmd = request("cmd") else cmd = ""

If len(trim(request("linhasDetalhe"))) < 1 Then linhasDetalhe = 10 Else linhasDetalhe = request("linhasDetalhe")

If len(trim(request("pagina"))) < 1 Then pagina = 1 Else pagina = Request("pagina")
	...
	<script language="JavaScript" type="text/javascript">
		...
		function permissoes() {
			...
			if (f.usuario.value>0) {
				...
				alert("É preciso selecionar um usuario!")
			}
		}

		function salvar() {
			if (validarEmail(document.forms[0].email.value) == true) {
		...
		function excluir() {
			...
			if (intDels>0) {
				if (confirm("Você está prestes a apagar " + intDels + " registros.\n Tem certeza que deseja continuar?")) {
					document.forms[0].action="usuariosadm_excluir.asp";
					document.forms[0].submit();
				}
			} else {
	...
	<input type="Hidden" name="ordemType" value="<%=ordemType%>">
		...
		<div id="principal">
			...
			<div id="janela">
				...
				<div id="titulo">Cadastro de Usuários Administradores</div>
			...
			<%	end if	%>
				...
				<div id="conteudo">
					<% If oper = "I" OR oper = "E" Then %>
					<!-- #include file="usuariosadm_incluir_editar.asp"-->
					<% Else %>
					<!-- #include file="usuariosadm_lista.asp"-->
					<% End If %>
				</div>
