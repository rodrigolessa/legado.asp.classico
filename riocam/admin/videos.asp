...
Dim query
Dim modelo_video, modelo, titulo, titulo_eng, status, dataCad
DIM nomeArquivo, tamanhoArquivo
DIM nomeArquivo2, tamanhoArquivo2
DIM tipo, extArquivo, nomeModelo, bolArquivo, bolArquivo2
...
<head>
	...
	function getOption() {
		...
		f.modelo_video.value = 0;
		if (f.registroSel.length != undefined) {
			for (var i = 0; i < f.registroSel.length; i++) {
				if (f.registroSel[i].checked) { f.modelo_video.value = f.registroSel[i].value }
			}
		} else {
		f.modelo_video.value = f.registroSel.value
		}
	}
	...
	function editarLinha(a) {
		...
		document.forms[0].modelo_video.value=a;
		document.forms[0].submit();
	}
	...
	function excluir() {
		...
		for (var i = 0; i < f.elements.length; i++) {
			if ((f.elements[i].name=="avideo") && (f.elements[i].checked)) {
				intDels++;
			}
		}
		if (intDels>0) {
			if (confirm("Você está prestes a apagar " + intDels + " registros.\n Tem certeza que deseja continuar?")) {
				document.forms[0].action="videos_excluir.asp";
				document.forms[0].submit();
			}
		} else {
	...
	function cadVideo() {
		...
		getOption();
		if (f.modelo_video.value>0) {
			var sURL		= "dlgVideo_incluir.asp?modelo_video="+f.modelo_video.value;
	...
	function cadVideoFoto() {
		...
		getOption();
		if (f.modelo_video.value>0) {
			var sURL		= "dlgVideoFoto_incluir.asp?modelo_video="+f.modelo_video.value;
			var	fotoMod		= window.open(sURL, "fotoMod", parametros);
				fotoMod.focus();
		} else {
			alert("É preciso selecionar um registro!");
...
-->	
		...
		<div id="principal">
			...
			<div id="janela">
				...
				<div id="titulo">Cadastro de vídeos</div>
				...
				<div id="ferramenta">
					...
					%>					
						...
						<img src="<%=dirImgsGerencia%>bt_cam_off.gif"		width=20	height=15	border=0	alt="Vídeo">
						<img src="<%=dirImgsGerencia%>ico_sep.gif"			width=2		height=18	border=0>
					<%
						Else
					%>
						...
						<img src="<%=dirImgsGerencia%>ico_sep.gif"			width=2		height=18	border=0>
						<a href="#" onclick="cadVideoFoto();	return false;"><img src="<%=dirImgsGerencia%>bt_imagem_on.gif"		width=20	height=15	border=0	alt="Imagem"></a>
						<a href="#" onclick="cadVideo();		return false;"><img src="<%=dirImgsGerencia%>bt_cam_on.gif"			width=20	height=15	border=0	alt="Vídeo"></a>
						<img src="<%=dirImgsGerencia%>ico_sep.gif"			width=2		height=18	border=0>
						linhas:
			...
			<%	end if	%>
				...
				<div id="conteudo">
					<% If oper = "I" OR oper = "E" Then %>
					<!-- #include file="videos_incluir_editar.asp"-->
					<% Else %>
					<!-- #include file="videos_lista.asp"-->
					<% End If %>
				</div>
